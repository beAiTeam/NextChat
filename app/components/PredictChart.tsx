...
const PredictChart = () => {
  ...
  const generateDetailsData = (items: PredictItem[]) => {
    ...
    return items.map((item, index) => {
      ...
      // 处理开奖号码和中奖号码的高亮
      const drawNumbersWithHighlight =
        item.ext_result?.map((result) => {
          ...
          const remainingDigits = numbers.filter(
            (digit) => digit !== firstDigitOfPrediction,
          );
          // 找到匹配的后四位数字
          const matchedLastDigits = lastFourDigits.filter((digit) =>
  ...
  const [baseGuessType, setBaseGuessType] = useState("ai_5_normal");
  const [compareGuessType, setCompareGuessType] = useState("ai_5_plus");
        ...
        <Space direction="vertical" size="large" style={{ width: "100%" }}>
          ...
          <Space>
            <div>
              ...
              <span>对比预测对比模型：</span>
              <Select
                value={compareGuessType}
                onChange={(value) => {
                  setCompareGuessType(value);
                }}
                options={[
                  { value: "ai_5_normal", label: "AI-5" },
                  { value: "ai_5_plus", label: "AI-5 Plus" },
          ...
          <PredictStats
            isCompare={isCompare}
            baseGuessType={baseGuessType}
            compareGuessType={compareGuessType}
          ...
          <WinLoseChart winLoseData={winLoseData} loading={loading} />

          <HeatmapChart heatmapData={heatmapData} loading={loading} />
